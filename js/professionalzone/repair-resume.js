...
function onLoadRepairResume() {
    ...
    let repairs = JSON.parse(sessionStorage.getItem("repairings"))
    let repair = repairs.filter(n => n.id == sessionStorage.getItem("repairId"))[0]
    ...
    document.querySelector('#start-chat').addEventListener('click', () => {
        if(!repair.interestedPersons.includes(-1))
        {
            ...
            sessionStorage.setItem("repairings", JSON.stringify(repairs))
        }
